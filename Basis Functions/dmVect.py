...
for dayNum in days:
	...
	inPhaseX[index]=np.cos(xAlt*np.pi/180)*np.cos((xAz+theta_d)*np.pi/180)
	outPhaseX[index]= np.cos(xAlt*np.pi/180)*np.sin((xAz+theta_d)*np.pi/180)
	index+=1
...
for dayNum in days:
	...
	inPhaseY[index]=np.cos(yAlt*np.pi/180)*np.cos((yAz+theta_d)*np.pi/180)
	outPhaseY[index]= np.cos(yAlt*np.pi/180)*np.sin((yAz+theta_d)*np.pi/180)
	index+=1
...
for dayNum in days:
	...
	inPhaseZ[index]=np.cos(zAlt*np.pi/180)*np.cos((zAz+theta_d)*np.pi/180)
	outPhaseZ[index]= np.cos(zAlt*np.pi/180)*np.sin((zAz+theta_d)*np.pi/180)
	index+=1
